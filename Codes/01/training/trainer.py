"""Training loop implementation for single and weighted loss strategies."""
...
from training.callbacks import TrainingTimeLogger, ValidationImageLogger
...
class Trainer:
    ...
    def __init__(
        ...
        self.optimizer = tf.keras.optimizers.Adam(
            learning_rate=float(config.get("training_learning_rate", 1e-4)),
            clipnorm=float(config.get("training_gradient_clip_norm", 1.0)),
        )
        self.strategy = str(config.get("loss_strategy", "single")).lower()

        epochs = int(config.get("training_epochs", 100))
        self.lr_scheduler = CosineAnnealingLR(
            base_lr=float(config.get("training_learning_rate", 1e-4)),
            min_lr=float(config.get("training_lr_scheduler_min_lr", 1e-7)),
            total_epochs=epochs,
            warmup_epochs=int(config.get("training_lr_scheduler_warmup_epochs", 0)),
        )
        self.plateau_scheduler = PlateauScheduler(
            initial_lr=float(config.get("training_learning_rate", 1e-4)),
            mode=str(config.get("training_early_stopping_mode", "max")),
        )
        self.early_stopping = MultiMetricEarlyStopping(
            monitor=str(config.get("training_early_stopping_monitor", "val_iou")),
            patience=int(config.get("training_early_stopping_patience", 15)),
            mode=str(config.get("training_early_stopping_mode", "max")),
            min_delta=float(config.get("training_early_stopping_min_delta", 1e-4)),
        )
        ...
        self.validation_image_logger = ValidationImageLogger(
            ...
            interval=int(config.get("logging_validation_image_interval", 5)),
        )
    ...
    def _restore_training_state(self) -> tuple[int, Dict[str, list], bool]:
        ...
        auto_resume = bool(checkpoint_cfg.get("auto_resume", False)) or bool(
            self.config.get("checkpointing_auto_resume", False)
        )
        if not auto_resume:
        ...
        if self.strategy == "mgda" and state.get("mgda_alpha_history"):
            pass  # MGDA removed; kept as a no-op guard so saved checkpoints aren't broken
    ...
    def _ensure_model_built(self) -> None:
        ...
        if self.model.built:
            return
        image_size = int(self.config.get("data_image_size", 256))
    ...
    def _set_lr(self, epoch: int) -> float:
        scheduler_type = str(self.config.get("training_lr_scheduler_type", "cosine")).lower()
    ...
    def fit(
        ...
        epochs = int(self.config.get("training_epochs", 100))
